...
//eerste
if($page > 2) echo '<a href="'.base_url().'/admin/listusers.php/?page=0">&lt;&lt; 
Eerste</a> ';

//n-2
if($page > 1) echo '<a href="'.base_url().'/admin/listusers.php/?page='.($page-2).'">'.($page-1).'</a> ';

//n-1
if($page > 0) echo '<a href="'.base_url().'/admin/listusers.php/?page='.($page-1).'">'.$page.'</a> ';

//pagina zelf
echo '<b>'.($page+1).'</b>';

//n+1
if($page < $max) echo ' <a href="'.base_url().'/admin/listusers.php/?page='.($page+1).'">'.($page+2).'</a>';

//n+2
if($page < $max-1) echo ' <a href="'.base_url().'/admin/listusers.php/?page='.($page+2).'">'.($page+3).'</a>';

//laatste
if($page < $max-2) echo ' <a href="'.base_url().'/admin/listusers.php/?page='.$max.'">&gt;&gt; Laatste</a>';


echo '<br /><br /><br />';

//nieuwsitems ophalen
	$result = mysql_query("SELECT * FROM `users` ORDER BY id DESC LIMIT ".$perpage." OFFSET ".$page*$perpage) or die("fout: " . mysql_error());

//nieuwsitems weergeven
while( $row = mysql_fetch_array($result) ){
    echo '<div class="h1">';
    echo '<a href="'.base_url().'/admin/edituser.php?id='.$row["id"].'"><img src="http://www.archon.org/edit.png" /></a> ';
    echo $row['username'].' ';
	if($row['active']==1) echo '<img src="http://www.lovento.com/media/images/icons/crystal/16/success.png" />';
	else echo '<img src="http://www.seoworld.com/images/b_drop.png" />';
    echo '</div><div class="newsbody">';
	echo 'e-mail: '.$row["e-mail"]." <br/>";
	echo 'admin: ';
	if($row["admin"]==1) echo "<b>yep</b>";
	else echo "nope";
	echo " <br/>";
    echo '</div>';
	echo "\n";
