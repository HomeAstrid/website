...
class UGCAS_Simple {
    var $cas_url = 'https://login.ugent.be/';
...
function ugcas_simple_remote_user() {
    if (!session_id()) session_start() or exit("can't start session");

    if ($user = $_SESSION['ugcas_simple_remote_user']) return $user;
    ...
    if ($ticket = $_GET['ticket']) {
        $user = $cas->service_validate($ticket);
        if ($user) {
            $_SESSION['ugcas_simple_remote_user'] = $user;
            return $user;
        }
        else {
            echo "<h1>ERROR</h1>\n";
